...
describe('Lottery', () => {
  ...
  it('not allow joins addresses with value different to default fee', async () => {
    try {
      await lottery.methods.join().send({
        from: accounts[1],
        gas: '1000000',
        value: convertFromGweiToWei(initialEntryFeeGwei - 1),
      })
      expect(true).toBe(false)
    } catch (e) {
      expect(e.message).toContain(
        'Exception while processing transaction: revert Insufficient tx value',
      )
    }
  })
  ...
  it('should increment lottery prize after joining', async () => {
    ...
    const lotteryAfterValue = await lottery.methods
      .getLotteryValue()
      .call({ from: accounts[2], gas: '1000000' })
  ...
  it('should allow multiple participants to join', async () => {
    ...
    const lotteryAfterValue = await lottery.methods
      .getLotteryValue()
      .call({ from: accounts[2], gas: '1000000' })
    expect(lotteryAfterValue).toBe(
      convertFromGweiToWei(initialEntryFeeGwei * 3),
    )
  })
  ...
  it('winner should get the prize', async () => {
    ...
    const diff = web3.utils
      .toBN(finalBalance)
      .sub(web3.utils.toBN(initialBalance))
